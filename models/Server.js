...
Schema.methods.parsePlugins = async function (plugins) {
	...
	await Promise.all(
		parsedPlugins.map(async description => {
...
Schema.methods.numplayersStats = async function (from, to, samplingInterval) {
	...
	var availableSamplingIntervals = ['1m', '5m', '10m', '15m', '30m', '1h', '3h', '6h', '12h', '1d', '2d', '3d', '7d'];
	if (!availableSamplingIntervals.includes(samplingInterval)) {
		throw new UnprocessableEntity(
			`time interval value must be one of (${availableSamplingIntervals.join(', ')}) but "${samplingInterval}" given`,
		);
	}
	...
	logger.info(
		`Getting stats from InfuxDB (server=${this.name} time=${from.format()}~time${to.format()} samplingInterval=${samplingInterval})`,
	);
...
Schema.methods.heartbeat = async function () {
	await Promise.all([this.query1(), this.query2()]);
};
